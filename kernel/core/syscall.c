...
static uint64 sys_ai_worker_register(void) {
    return (uint64)ai_service_worker_register();
}

static uint64 sys_ai_worker_get(void) {
    struct proc *p = myproc();
    if (p == 0) {
        return (uint64)-1;
    }
    return (uint64)ai_service_worker_get(
        p->trapframe->a0,
        (int)p->trapframe->a1,
        p->trapframe->a2,
        p->trapframe->a3
...
static uint64 sys_ai_worker_complete(void) {
    struct proc *p = myproc();
    if (p == 0) {
        return (uint64)-1;
    }
    return (uint64)ai_service_worker_complete(
        (int)p->trapframe->a0,
        p->trapframe->a1,
        (int)p->trapframe->a2,
        (int)p->trapframe->a3
...
static uint64 (*syscalls[])(void) = {
    ...
    [SYS_ai_worker_register] = sys_ai_worker_register,
    [SYS_ai_worker_get] = sys_ai_worker_get,
    [SYS_ai_worker_complete] = sys_ai_worker_complete,
};
