...
static int current_log_enabled = 1;
...
int llm_log_enabled(void) {
    return current_log_enabled;
}

void llm_set_log_enabled(int enabled) {
    current_log_enabled = enabled ? 1 : 0;
}
...
uint64 llm_now_raw_time(void) {
    uint64 x;
    asm volatile("csrr %0, time" : "=r"(x));
    return x;
}

uint64 llm_raw_time_to_us(uint64 raw_delta) {
    return (raw_delta * 1000000ULL + (LLM_TIME_HZ / 2ULL)) / LLM_TIME_HZ;
}

void llm_print_ms_value(uint64 us) {
    uint64 whole = us / 1000ULL;
    uint64 frac = us % 1000ULL;

    printf("%llu.", (unsigned long long)whole);
    if (frac < 100ULL) {
        printf("0");
    }
    if (frac < 10ULL) {
    ...
    printf("%llu", (unsigned long long)frac);
}

void llm_print_rate_value(uint64 x100) {
    uint64 whole = x100 / 100ULL;
    uint64 frac = x100 % 100ULL;
